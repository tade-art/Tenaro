...
export default function HowItWorks() {
  return (
    <section className="section-wrapper text-center">
      <p className="section-subheading text-[var(--color-subtle-text)]">How It Works</p>
      ...
      <div className="mt-10 grid gap-10 md:grid-cols-3 text-left max-w-6xl mx-auto">
        {stepList.map((step, i) => (
          <motion.div
            key={step.title}
            className="feature-card bg-[var(--color-card-bg)] border border-[var(--color-border)] rounded-xl p-6 shadow-md"
          ...
          >
            ...
            <p className="text-[var(--color-subtle-text)]">{step.desc}</p>
          </motion.div>
        ))}
      </div>
    </section>
